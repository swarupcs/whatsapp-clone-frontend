...
import { useEditMessage, messageKeys } from '@/hooks/queries/useMessages';
...
export default function UndoToast({ conversationId }: Props) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Date.now();
      undoDeleteStack.forEach((entry) => { if (entry.expiresAt < now) popUndoDelete(entry.message.id); });
      undoEditStack.forEach((entry) => { if (entry.expiresAt < now) popUndoEdit(entry.messageId); });
    }, 1000);
    return () => clearInterval(interval);
  }, [undoDeleteStack, undoEditStack]);
  const handleUndoDelete = (messageId: string) => {
    ...
    queryClient.setQueryData(messageKeys.list(conversationId), (old: any) => {
      if (!old) return old;
      return { ...old, pages: old.pages.map((page: PaginatedData<Message>) => ({ ...page, data: page.data.map((m) => m.id === messageId ? { ...m, isDeleted: false, deletedAt: undefined } : m) })) };
    });
  };
  ...
  const now = Date.now();
  const allToasts = [
    ...undoDeleteStack.filter((e) => e.expiresAt > now).map((e) => ({ id: `del-${e.message.id}`, type: 'delete' as const, messageId: e.message.id, label: 'Message deleted' })),
    ...undoEditStack.filter((e) => e.expiresAt > now).map((e) => ({ id: `edit-${e.messageId}`, type: 'edit' as const, messageId: e.messageId, label: 'Message edited' })),
  ];
      ...
      <AnimatePresence>
        {allToasts.map((toast) => (
          <motion.div key={toast.id} initial={{ opacity: 0, y: 20, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.9 }}
            ...
            <Button size='sm' variant='ghost' className='h-7 px-2 gap-1 text-primary hover:text-primary hover:bg-primary/10'
              onClick={() => toast.type === 'delete' ? handleUndoDelete(toast.messageId) : handleUndoEdit(toast.messageId)}>
