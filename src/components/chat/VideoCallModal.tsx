...
import { X, Mic, MicOff, Video, VideoOff, Monitor, PhoneOff, MessageSquare, Maximize2, Minimize2 } from 'lucide-react';
...
export default function VideoCallModal({ isOpen, onClose, contactName, contactAvatar }: Props) {
  ...
  useEffect(() => {
    if (isOpen && isVideoOn) startCamera();
    return stopCamera;
  }, [isOpen]);

  useEffect(() => {
    let iv: ReturnType<typeof setInterval>;
    if (isOpen) iv = setInterval(() => setCallDuration((d) => d + 1), 1000);
    return () => { clearInterval(iv); setCallDuration(0); };
  }, [isOpen]);
  ...
  const startCamera = async () => {
    ...
    } catch { toast.error('Camera unavailable'); setIsVideoOn(false); }
  };

  const stopCamera = () => { stream?.getTracks().forEach((t) => t.stop()); setStream(null); };

  const toggleVideo = async () => {
    if (isVideoOn) { stream?.getVideoTracks().forEach((t) => (t.enabled = false)); setIsVideoOn(false); }
    else { if (stream) stream.getVideoTracks().forEach((t) => (t.enabled = true)); else await startCamera(); setIsVideoOn(true); }
  };

  const toggleMute = () => { stream?.getAudioTracks().forEach((t) => (t.enabled = isMuted)); setIsMuted(!isMuted); };

  const toggleScreenShare = async () => {
    if (isScreenSharing) { setIsScreenSharing(false); if (isVideoOn) await startCamera(); toast.success('Screen sharing stopped'); }
    else {
      try {
        ...
        ss.getVideoTracks()[0]!.onended = () => { setIsScreenSharing(false); if (isVideoOn) startCamera(); };
        ...
        <div ref={containerRef} className='relative w-full h-full flex flex-col'>
            ...
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-3'>
                ...
                <div>
                  ...
                  <p className='text-sm text-white/70'>{fmt(callDuration)}</p>
                </div>
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className='flex items-center justify-center gap-4'>
              {[
                { onClick: toggleMute, active: isMuted, icon: isMuted ? MicOff : Mic },
                { onClick: toggleVideo, active: !isVideoOn, icon: isVideoOn ? Video : VideoOff },
                { onClick: toggleScreenShare, active: isScreenSharing, icon: Monitor, isBlue: true },
              ].map(({ onClick, active, icon: Icon, isBlue }, i) => (
                <motion.div key={i} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Button variant='secondary' size='lg' onClick={onClick}
                    className={cn('h-14 w-14 rounded-full',
                      active && !isBlue && 'bg-destructive hover:bg-destructive/90 text-white',
                      active && isBlue && 'bg-primary hover:bg-primary/90 text-white')}>
