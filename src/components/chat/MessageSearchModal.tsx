...
export default function MessageSearchModal({ open, onOpenChange }: Props) {
  ...
  // Enrich messages with conversation + sender
  const results = messages
    ...
    .filter(Boolean) as { message: Message; conversation: (typeof conversations)[0]; sender: (typeof conversations)[0]['users'][0] | undefined }[];
  ...
  const handleResultClick = useCallback(
    ...
    [conversations, setActiveConversation, markRead, onOpenChange],
  );
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className='sm:max-w-lg p-0 gap-0 bg-card border-border overflow-hidden'>
        <DialogHeader className='p-4 pb-0'>
          <DialogTitle className='text-lg font-semibold'>Search Messages</DialogTitle>
        </DialogHeader>
                ...
                <AnimatePresence mode='popLayout'>
                  {results.map((result, index) => (
                    <motion.button
                      ...
                      className={cn(
                        'w-full text-left p-3 rounded-lg hover:bg-secondary/80 transition-colors group',
                    ...
                    >
                      <div className='flex items-start gap-3'>
                        <Avatar className='h-10 w-10 shrink-0'>
                          ...
                          <AvatarFallback>{result.sender?.name?.[0] ?? '?'}</AvatarFallback>
                        </Avatar>
                        <div className='flex-1 min-w-0'>
                          <div className='flex items-center justify-between gap-2 mb-1'>
                            <span className='font-medium text-sm truncate'>
                              {result.sender?.id === user?.id ? 'You' : result.sender?.name ?? 'Unknown'}
                            </span>
                            <span className='text-xs text-muted-foreground shrink-0'>
                              {format(new Date(result.message.createdAt), 'MMM d, HH:mm')}
                            </span>
                          </div>
