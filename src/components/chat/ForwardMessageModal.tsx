...
export default function ForwardMessageModal({ isOpen, onClose, message }: Props) {
  ...
  const handleForward = async () => {
    ...
    await Promise.all(
      selected.map((convId) =>
        forwardMutation.mutateAsync({ messageId: message.id, toConversationId: convId }),
      ),
    );
    setSelected([]);
    setSearchQuery('');
      ...
      <DialogContent className='sm:max-w-md'>
        ...
        {message && (
          <div className='p-3 rounded-lg bg-secondary/50 border border-border'>
            ...
            {message.files && message.files.length > 0 && (
              <p className='text-xs text-muted-foreground mt-1'>+ {message.files.length} attachment(s)</p>
            )}
          </div>
        )}
                ...
                <AnimatePresence key={conv.id}>
                  <motion.div
                    ...
                    className={cn(
                      ...
                      selected.includes(conv.id) ? 'bg-primary/10 border border-primary/30' : 'hover:bg-secondary',
                    )}
                  ...
                  >
                    ...
                    <div className='flex-1 min-w-0'>
                      ...
                      <p className='text-xs text-muted-foreground'>
                        {conv.isGroup ? `${conv.users.length} members` : 'Direct message'}
                      </p>
                    </div>
        ...
        <div className='flex items-center justify-between pt-4 border-t border-border'>
          <p className='text-sm text-muted-foreground'>
            {selected.length > 0 ? `${selected.length} selected` : 'Select conversations'}
          </p>
          <Button onClick={handleForward} disabled={selected.length === 0 || forwardMutation.isPending}
            className='gap-2'>
