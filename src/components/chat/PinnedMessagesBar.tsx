...
import { usePinnedMessages } from '@/hooks/queries/useMessages';
import { usePinMessage } from '@/hooks/queries/useMessages';
          ...
          <motion.div key='collapsed' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className='flex items-center gap-1 text-primary'>
              ...
              {pinnedMessages.length > 1 && (
                <span className='text-xs font-medium'>{currentIndex + 1}/{pinnedMessages.length}</span>
              )}
            </div>
            <button
              ...
              {pinnedMessages.length > 1 && (
                <>
                  <Button variant='ghost' size='icon' className='h-6 w-6'
                    onClick={() => setCurrentIndex((p) => (p - 1 + pinnedMessages.length) % pinnedMessages.length)}>
                    <ChevronUp className='h-3 w-3' />
                  </Button>
                  <Button variant='ghost' size='icon' className='h-6 w-6'
                    onClick={() => setCurrentIndex((p) => (p + 1) % pinnedMessages.length)}>
              ...
              {pinnedMessages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className='px-4 py-2 flex items-start gap-3 hover:bg-muted/50 transition-colors group'>
                    ...
                    <button onClick={() => { onScrollToMessage(msg.id); setIsExpanded(false); }}
                      className='flex-1 min-w-0 text-left'>
                      <span className='text-xs font-medium'>
                        {msgSender?.id === currentUserId ? 'You' : msgSender?.name}
                      </span>
                      <p className='text-sm text-muted-foreground line-clamp-2'>{msg.message}</p>
                    </button>
                    <Button variant='ghost' size='icon' className='h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity shrink-0'
                      onClick={() => pinMutation.mutate({ messageId: msg.id, pin: false })}>
                      <X className='h-3 w-3' />
                    </Button>
                  </div>
                );
