...
export default function MessageActions({ messageId, messageText, isOwn, isPinned, onEdit, onDelete, onForward, onReply, onPin, onUnpin, isVisible }: Props) {
  ...
  const handleSave = () => {
    if (editValue.trim() && editValue !== messageText) onEdit(messageId, editValue.trim());
    setIsEditing(false);
  };
  ...
  if (isEditing) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
        ...
        <Input value={editValue} onChange={(e) => setEditValue(e.target.value)} onKeyDown={handleKey}
          className='flex-1 h-8 text-sm bg-secondary border-0' autoFocus />
            ...
            <DropdownMenuContent align={isOwn ? 'end' : 'start'} className='w-32'>
              ...
              {isOwn && (
                <>
                  ...
                  <DropdownMenuItem onClick={() => { setEditValue(messageText); setIsEditing(true); }} className='gap-2'>
                    <Pencil className='h-4 w-4' />Edit
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => onDelete(messageId)} className='gap-2 text-destructive focus:text-destructive'>
