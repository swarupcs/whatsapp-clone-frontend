    ...
    <div className='flex items-center gap-3 min-w-[200px]'>
      <Button variant='ghost' size='icon' onClick={toggle}
        className={cn('h-10 w-10 rounded-full shrink-0',
          isOwn ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-primary/20 hover:bg-primary/30 text-primary')}>
            ...
            return (
              <motion.div key={i}
                className={cn('w-0.5 rounded-full transition-colors',
                  active ? (isOwn ? 'bg-white' : 'bg-primary') : (isOwn ? 'bg-white/40' : 'bg-muted-foreground/40'))}
                style={{ height: `${h * 100}%` }}
                animate={isPlaying && active ? { scaleY: [1, 1.2, 1] } : {}}
