...
export default function FilePreviewScreen({ isOpen, onClose, files, onSend, onAddMore, onRemove }: Props) {
  ...
  useEffect(() => {
    if (files.length > 0 && activeIndex >= files.length) setActiveIndex(Math.max(0, files.length - 1));
  ...
  useEffect(() => {
    files.forEach(async (file, i) => {
      ...
      if (cat === 'IMAGE') setPreviews((p) => ({ ...p, [i]: URL.createObjectURL(file) }));
        ...
        try { const t = await generateVideoThumbnail(file); setPreviews((p) => ({ ...p, [i]: t })); } catch {}
      }
    });
  }, [files]);
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <div className='flex-1 relative flex items-center justify-center p-8 overflow-hidden'>
          {files.length > 1 && (
            <>
              <Button variant='ghost' size='icon'
                className={cn('absolute left-4 z-10 h-12 w-12 rounded-full bg-card/50', activeIndex === 0 && 'opacity-50 pointer-events-none')}
                onClick={() => setActiveIndex((p) => Math.max(0, p - 1))}>
                <ChevronLeft className='h-6 w-6' />
              </Button>
              <Button variant='ghost' size='icon'
                className={cn('absolute right-4 z-10 h-12 w-12 rounded-full bg-card/50', activeIndex === files.length - 1 && 'opacity-50 pointer-events-none')}
          ...
          <motion.div key={activeIndex} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.2 }} className='max-w-full max-h-full'>
            {cat === 'IMAGE' && previews[activeIndex] ? (
              <img src={previews[activeIndex]} alt={active.name}
                className='max-h-[60vh] max-w-full object-contain rounded-lg shadow-2xl' />
            ) : cat === 'VIDEO' ? (
              <video src={URL.createObjectURL(active)} controls
              ...
              <div className='flex flex-col items-center justify-center p-12 bg-card rounded-2xl border border-border'>
                <div className={cn('h-24 w-24 rounded-2xl flex items-center justify-center mb-4', info.color, 'bg-current/10')}>
                  <Icon className={cn('h-12 w-12', info.color)} />
                </div>
                <h3 className='text-lg font-medium text-center mb-2'>{active.name}</h3>
                <p className='text-sm text-muted-foreground'>{formatFileSize(active.size)}</p>
              </div>
            )}
          </motion.div>
        </div>
        ...
        {files.length > 1 && (
          <div className='px-4 py-3 border-t border-border'>
            <div className='flex gap-2 overflow-x-auto scrollbar-thin justify-center'>
              {files.map((f, i) => {
                const fi = getFileTypeInfo(f.type);
                const fc = getFileCategory(f.type);
                const FI = fi.icon;
                return (
                  <button key={i} onClick={() => setActiveIndex(i)}
                    className={cn('relative shrink-0 h-16 w-16 rounded-lg overflow-hidden border-2 transition-all group',
                      activeIndex === i ? 'border-primary shadow-lg shadow-primary/20' : 'border-border hover:border-muted-foreground')}>
                    {(fc === 'IMAGE' || fc === 'VIDEO') && previews[i] ? (
                      <img src={previews[i]} alt={f.name} className='h-full w-full object-cover' />
                    ) : (
                      <div className={cn('h-full w-full flex items-center justify-center bg-muted', fi.color)}>
                        <FI className='h-6 w-6' />
                      </div>
                    )}
        ...
        <div className='p-4 border-t border-border bg-card/50 backdrop-blur-sm'>
          <div className='flex items-center gap-3 max-w-2xl mx-auto'>
            ...
            <Button onClick={handleSend} disabled={isSending} className='h-12 w-12 rounded-full gradient-glow'>
