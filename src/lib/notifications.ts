class NotificationService {
  ...
  async showNotification(title: string, options?: { body?: string; icon?: string; tag?: string; onClick?: () => void }): Promise<void> {
    if (this.soundEnabled) this.playSound();
    if (this.permission !== 'granted') return;
    try {
      const n = new Notification(title, { body: options?.body, icon: options?.icon ?? '/favicon.ico', tag: options?.tag, silent: true });
  ...
  playSound(): void {
    ...
    try { this.audio.currentTime = 0; this.audio.play().catch(() => {}); } catch { /* blocked */ }
  }
